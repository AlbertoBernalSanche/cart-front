...
export class CartComponent implements OnInit {
  ...
  public findShoppingProductsByShoppingCart(): void {
    ...
    this.cartService.findShoppingProductByShoppingCart(this.carId).subscribe(async data => {
      this.shoppingProducts = await data;
      for (let index = 0; index < this.shoppingProducts.length; index++) {

        this.productService.findById(this.shoppingProducts[index].productId).subscribe(async data1 => {
          await this.products.push(data1);
          //this.products[index]= await  data1
